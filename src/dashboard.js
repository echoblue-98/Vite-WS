...
(function dashboardModule() {
  ...
  // Chart size and margin constants
  const TIMELINE_WIDTH = 500, TIMELINE_HEIGHT = 250;
  const HISTOGRAM_WIDTH = 500, HISTOGRAM_HEIGHT = 250;
  const HEATMAP_WIDTH = 500;
  const TIMELINE_MARGIN = {top: 20, right: 30, bottom: 30, left: 40};
  const HISTOGRAM_MARGIN = {top: 20, right: 30, bottom: 30, left: 40};
  const HEATMAP_MARGIN = {top: 20, right: 30, bottom: 30, left: 100};
  ...
  // Attach event listener only if fileInput exists
  if (!fileInput) {
    console.error('fileInput element not found. Dashboard will not function.');
    return;
  } else {
  ...
  /**
   * Handles file input change event: reads and parses uploaded session log files.
   */
  ...
  /**
   * Computes the average load/play time for a session run.
   */
  function avgRun(run) {
  ...
  /**
   * Logs a message to the overlay (if present) or to the console.
   */
  function voLog(msg) {
  ...
  /**
   * Checks for baseline drift in the latest session and flags if out of tolerance.
   */
  ...
  /**
   * Renders all dashboard visualizations (timeline, histogram, heatmap).
   */
  ...
  /**
   * Renders the timeline chart of session averages and outlier markers.
   */
  function renderTimeline() {
    const margin = TIMELINE_MARGIN;
    const width = TIMELINE_WIDTH, height = TIMELINE_HEIGHT;
  ...
  /**
   * Renders the histogram of all load/play times across sessions.
   */
  function renderHistogram() {
    ...
    const width = HISTOGRAM_WIDTH, height = HISTOGRAM_HEIGHT, margin = HISTOGRAM_MARGIN;
  ...
  /**
   * Renders the heatmap of cue usage frequency across sessions.
   */
  function renderHeatmap() {
    ...
    const width = HEATMAP_WIDTH, height = cues.length * 20, margin = HEATMAP_MARGIN;
